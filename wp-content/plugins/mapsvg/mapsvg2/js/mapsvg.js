    ...
    var defaults = {
        ...
        menu: {
            ...
            template: function (region) {
                return (
                    '<li><a href="#' + region.id + '">' + (region.title || region.id) + "</a></li>"
                );
            },
        },
        menuMarkers: {
    ...
    mapSVG = function (elem, options) {
        ...
        this.methods = {
                ...
                if (!error) return func;
                    ...
                    return {
                        error: {
                            line: error.line,
                            text:
                                "MapSVG user function error: (line " +
                                error.line +
                                "): " +
                                error.message,
                        },
                        ...
                        $.each(options.markers, function (id, markerOptions) {
                            ...
                            if (markerOptions.geoCoords) {
                                if (typeof markerOptions.geoCoords == "string") {
                                    markerOptions.geoCoords = markerOptions.geoCoords
                                        .trim()
                                        .split(",");
                ...
                switch (_data.options.tooltips.position) {
                    ...
                    case "bottom-left":
                        $("body").on(event, function (e) {
                            window.m = mouseCoords(e);
                            tip.style.left =
                                e.clientX + $(window).scrollLeft() - tip.offsetWidth + 10 + "px";
                            tip.style.top = e.clientY + $(window).scrollTop() + 25 + "px";
                        });
                        break;
                    case "bottom":
                        $("body").on(event, function (e) {
                            window.m = mouseCoords(e);
                            tip.style.left =
                                e.clientX + $(window).scrollLeft() - tip.offsetWidth / 2 + "px";
                            tip.style.top = e.clientY + $(window).scrollTop() + 25 + "px";
                        });
                        break;
                    case "top":
                        ...
                        $("body").on(event, function (e) {
                            ...
                            tip.style.left =
                                e.clientX + $(window).scrollLeft() - tip.offsetWidth / 2 + "px";
                            tip.style.top =
                                e.clientY + $(window).scrollTop() - tip.offsetHeight - 10 + "px";
                        });
                        break;
                    case "top-right":
                        $("body").on(event, function (e) {
                    ...
                    case "left":
                        $("body").on(event, function (e) {
                            ...
                            tip.style.left =
                                e.clientX + $(window).scrollLeft() - tip.offsetWidth - 10 + "px";
                            tip.style.top =
                                e.clientY + $(window).scrollTop() - tip.offsetHeight / 2 + "px";
                        });
                        break;
                    case "right":
                        $("body").on(event, function (e) {
            ...
            setPopovers: function (options) {
                ...
                if (!_data.mapPopover) {
                    ...
                    _data.mapPopover.css({
                        ...
                        height:
                            _data.options.popovers.height +
                            (_data.options.popovers.height == "auto" ? "" : "px"),
                    });
            ...
            setViewBox: function (v, skipAdjustments) {
                ...
                var isZooming =
                    parseInt(d[2]) != _data.viewBox[2] || parseInt(d[3]) != _data.viewBox[3];
                _data.viewBox = [
                    parseFloat(d[0]),
                    parseFloat(d[1]),
                    parseFloat(d[2]),
            ...
            setZoom: function (options) {
                ...
                options.mousewheel != undefined &&
                    (options.mousewheel = parseBoolean(options.mousewheel));
            ...
            setGauge: function (options) {
                ...
                if (options.colors) {
                    _data.options.gauge.colors.lowRGB = tinycolor(
                        _data.options.gauge.colors.low
                    ).toRgb();
                    _data.options.gauge.colors.highRGB = tinycolor(
            ...
            viewBoxGetBySize: function (width, height) {
                ...
                if (new_ratio != old_ratio) {
                    ...
                    if (new_ratio > old_ratio) {
                        ...
                        vb[0] =
                            _data.svgDefault.viewBox[0] - (vb[2] - _data.svgDefault.viewBox[2]) / 2;
                    } else {
                        vb[3] = _data.svgDefault.viewBox[2] / new_ratio;
                        vb[1] =
                            _data.svgDefault.viewBox[1] - (vb[3] - _data.svgDefault.viewBox[3]) / 2;
                    }
                }
            ...
            mapAdjustStrokes: function () {
                _data.regions.forEach(function (region) {
                    if (region.svg_style["stroke-width"]) {
                        region.node.css(
                            "stroke-width",
                            region.svg_style["stroke-width"] / _data.scale
                        );
                    }
                });
            },
            ...
            convertGeoToPixel: function (coords) {
                ...
                var x =
                    (lon - _data.geoViewBox.leftLon) * (_data.svgDefault.width / _data.mapLonDelta);

                var lat = (lat * 3.14159) / 180;
                var worldMapWidth =
                    ((_data.svgDefault.width / _data.mapLonDelta) * 360) / (2 * 3.14159);
                var mapOffsetY =
                    (worldMapWidth / 2) *
                    Math.log(
            ...
            convertPixelToGeo: function (tx, ty) {
                ...
                var worldMapRadius =
                    ((_data.svgDefault.width / _data.mapLonDelta) * 360) / (2 * Math.PI);
                var mapOffsetY =
                    (worldMapRadius / 2) *
                    Math.log(
                        (1 + Math.sin(_data.mapLatBottomDegree)) /
                ...
                var long =
                    _data.geoViewBox.leftLon + (tx / _data.svgDefault.width) * _data.mapLonDelta;
            ...
            setEventHandlers: function () {
                ...
                if (_data.editMarkers.on) {
                    var event2 = touchDevice
                        ? "touchstart.common.mapsvg"
                        : "mousedown.common.mapsvg";
            ...
            init: function (opts, elem) {
                ...
                var loading = $("<div>" + _data.options.loadingText + "</div>").addClass(
                    "mapsvg-loading"
                );
