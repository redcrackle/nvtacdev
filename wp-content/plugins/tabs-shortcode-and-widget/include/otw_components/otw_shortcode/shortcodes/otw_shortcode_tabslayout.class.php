...
class OTW_Shortcode_TabsLayout extends OTW_Shortcodes{
	...
	public function build_shortcode_editor_custom_options(){
		...
		$html .= OTW_Form::text_input( array( 'id' => 'otw-shortcode-element-css_id', 'label' => $this->get_label( 'Layout ID' ), 'description' => $this->get_label( 'If you use miltiple tab layouts at one page please specify id for each of them. The id should be string with only letters or numbers' ), 'parse' => $source )  );
	...
	public function build_shortcode_code( $attributes ){
		...
		if( !$this->has_error ){
			...
			$code .= $this->format_attribute( 'css_id', 'css_id', $attributes, false, '', true  );
	...
	public function display_shortcode( $attributes, $content ){
		...
		$tab_id = $this->format_attribute( '', 'css_id', $attributes, false, '' );
		...
		if( !empty( $tab_id ) ){
			$tab_id_prefix = $tab_id_prefix .= $tab_id.'-';
		}
		...
		if( $tabs = $this->format_attribute( '', 'tabs', $attributes, false, '' ) ){
			...
			for( $cT = 1; $cT <= $tabs; $cT++ ){
				...
				$html .= '<a href="#'.$tab_id_prefix.'tabs-'.$cT.'"';
				$html .= '>';
		...
		if( $tabs = $this->format_attribute( '', 'tabs', $attributes, false, '' ) ){
			...
			for( $cT = 1; $cT <= $tabs; $cT++ ){
				...
				$html .= '<div id="'.$tab_id_prefix.'tabs-'.$cT.'" class="ui-tabs-panel ui-widget-content ui-corner-bottom">';
